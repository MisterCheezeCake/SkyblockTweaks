...
public class HudScreen extends Screen {
    ...
    public HudScreen(MutableComponent title, ArrayList<HUD> huds, Screen parent) {
        super(title);
...
//                }
    ...
    private void drawInstructions(GuiGraphics guiGraphics) {
        var centerX = Minecraft.getInstance().getWindow().getGuiScaledWidth() / 2;
//        guiGraphics.getMatrices().push();
//        guiGraphics.getMatrices().translate(0, 0, 100);
        if (shouldShowText() && this.mode == Mode.DRAG) {
            guiGraphics.drawCenteredString(Minecraft.getInstance().font, "Drag or use the arrow keys to move items", centerX, 5, Colors.WHITE);
            guiGraphics.drawCenteredString(Minecraft.getInstance().font, "Scroll or use the + and - keys to scale items", centerX, 15, Colors.WHITE);
            guiGraphics.drawCenteredString(Minecraft.getInstance().font, "Press shift and hover to see the name of the item", centerX, 25, Colors.WHITE);
            guiGraphics.drawCenteredString(Minecraft.getInstance().font, (macOS() ? "Command" : "Control") + " click for text mode/to edit anchor points", centerX, 35, Colors.WHITE);
            guiGraphics.drawCenteredString(Minecraft.getInstance().font, (macOS() ? "Command" : "Control") + "+ R to enter Reset Mode", centerX, 45, Colors.WHITE);
            guiGraphics.drawCenteredString(Minecraft.getInstance().font, "Hold " + (macOS() ? "option" : "alt") + " to hide this text or " + (macOS() ? "Cmd+Opt" : "Ctrl+Alt") + " to toggle it", centerX, 55, Colors.WHITE);
        } else if (shouldShowText() && this.mode == Mode.TEXT) {
            guiGraphics.drawCenteredString(Minecraft.getInstance().font, "You are now in exact positioning mode", centerX, 5, Colors.WHITE);
            guiGraphics.drawCenteredString(Minecraft.getInstance().font, "Enter the x and y positions in the text fields below", centerX, 15, Colors.WHITE);
            guiGraphics.drawCenteredString(Minecraft.getInstance().font, "The number is relative, so 0 is fully up/left and 1 is fully down/right", centerX, 25, Colors.WHITE);
            guiGraphics.drawCenteredString(Minecraft.getInstance().font, "Press the anchor button to change where the hud anchors itself", centerX, 35, Colors.WHITE);
            guiGraphics.drawCenteredString(Minecraft.getInstance().font, "Hold " + (macOS() ? "option" : "alt") + " to hide this text or " + (macOS() ? "Cmd+Opt" : "Ctrl+Alt") + " to toggle it", centerX, 45, Colors.WHITE);
        } else if (this.mode == Mode.RESET) {
            guiGraphics.drawCenteredString(Minecraft.getInstance().font, "You are now in reset mode", centerX, 5, Colors.WHITE);
            guiGraphics.drawCenteredString(Minecraft.getInstance().font, "Press the reset button to reset the selected item to the default position", centerX, 15, Colors.WHITE);
            guiGraphics.drawCenteredString(Minecraft.getInstance().font, "Press the first button to cycle between elements", centerX, 25, Colors.WHITE);
        }
